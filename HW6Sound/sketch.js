...
let samplePlayers = {}; // Store separate players
let currentSample = null; // Keep track of the currently playing sample
...
function playSample(sampleName) {
  if (currentSample) {
    currentSample.stop(Tone.now()); // Stop currently playing sample
  }
  ...
  player.start();
  lastSample = player;
  currentSample = player; // Track the active sample
}
...
function stopAllSamples() {
  if (currentSample) {
    currentSample.stop(Tone.now()); // Stop the currently playing sample
  }
